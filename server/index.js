...
// Extract just the origin (scheme + host) from KOHA_OPAC_URL in case it includes a path
...
// ---------- IN-MEMORY CACHE ----------
// Two-tier cache: MARCXML records cached 1h, search results cached 5min.
// In-flight deduplication prevents parallel identical requests from all hitting Koha.
const MARC_TTL    = 60 * 60 * 1000;   // 1 hour
const SEARCH_TTL  = 5  * 60 * 1000;   // 5 minutes
const marcCache   = new Map(); // key: biblionumber  → { data, exp }
const searchCache = new Map(); // key: query string  → { data, exp }
const inFlight    = new Map(); // key: url           → Promise<string>
...
// Deduplicated fetch: concurrent callers for the same URL share one HTTP request.
...
app.post('/api/auth/register', authLimiter, (req, res) => {
  ...
  // DEF-004: enforce minimum password length server-side
  ...
  try {
    ...
    logAudit(result.lastInsertRowid, 'register', 'user', result.lastInsertRowid, email, req.ip);
...
app.post('/api/auth/login', authLimiter, (req, res) => {
  ...
  // DEF-019: fetch only needed columns, not SELECT *
  ...
  if (user.role === 'public' && user.membership_status === 'pending') {
    return res.status(403).json({ error: 'Your membership is pending approval. Please contact the library.' });
  }
  if (user.role === 'public' && user.membership_status === 'suspended') {
    return res.status(403).json({ error: 'Your membership has been suspended. Please contact the library.' });
  }
  const token = signToken(user);
  // DEF-007: add secure: true in production so cookie is HTTPS-only
  res.cookie('auth_token', token, { httpOnly: true, sameSite: 'lax', secure: IS_PROD, maxAge: 7 * 24 * 3600 * 1000 });
  logAudit(user.id, 'login', 'user', user.id, email, req.ip);
  res.json({
    success: true,
    user: { id: user.id, name: user.full_name, email: user.email, role: user.role,
            membership_id: user.membership_id, membership_status: user.membership_status }
  });
});

app.post('/api/auth/logout', (req, res) => {
  // DEF-009: revoke the token so it cannot be reused within its 7-day TTL
...
// ---------- BOOKS / CATALOG ----------
// DEF-016: safely parse and clamp limit/offset to prevent 500 on non-integer input
...
// DEF-006: reject cover_image values that use javascript: or data: schemes
...
app.get('/api/books', (req, res) => {
  ...
  const sql = `SELECT * FROM books${where} ORDER BY added_at DESC LIMIT ? OFFSET ?`;
...
app.get('/api/books/new-arrivals', (req, res) => {
  const books = db.prepare('SELECT * FROM books ORDER BY added_at DESC LIMIT 8').all();
  res.json({ books });
});

// ---------- CSV EXPORT (DEF-002) — must be before /:id wildcards ----------
function toCsv(rows) {
...
app.get('/api/books/export', authenticate, authorize('admin', 'librarian'), (req, res) => {
...
app.post('/api/books', authenticate, authorize('admin', 'librarian'), (req, res) => {
  ...
  // DEF-006: block javascript: and other script-injection URLs
  ...
  logAudit(req.user.id, 'create', 'book', result.lastInsertRowid, title, req.ip);
...
app.put('/api/books/:id', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const book = db.prepare('SELECT * FROM books WHERE id = ?').get(req.params.id);
  if (!book) return res.status(404).json({ error: 'Book not found' });
  // DEF-006: block javascript: and other script-injection URLs on update too
  if (req.body.cover_image !== undefined && !isSafeImageUrl(req.body.cover_image)) {
    return res.status(400).json({ error: 'Invalid cover_image URL' });
  }
  const fields = ['isbn','title','author','publisher','publication_year','category','collection_type',
                  'language','call_number','description','cover_image','total_copies','branch'];
  const updates = [];
  const params = [];
  ...
  logAudit(req.user.id, 'update', 'book', req.params.id, null, req.ip);
  res.json({ success: true });
});
...
app.delete('/api/books/:id', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const book = db.prepare('SELECT * FROM books WHERE id = ?').get(req.params.id);
  if (!book) return res.status(404).json({ error: 'Book not found' });
  const activeLoans = db.prepare("SELECT COUNT(*) AS c FROM loans WHERE book_id = ? AND status = 'active'").get(req.params.id).c;
  if (activeLoans > 0) return res.status(400).json({ error: 'Cannot delete book with active loans' });
  db.prepare('DELETE FROM reservations WHERE book_id = ?').run(req.params.id);
  db.prepare('DELETE FROM loans WHERE book_id = ?').run(req.params.id);
  db.prepare('DELETE FROM books WHERE id = ?').run(req.params.id);
  logAudit(req.user.id, 'delete', 'book', req.params.id, book.title, req.ip);
  res.json({ success: true });
});
...
app.post('/api/loans', authenticate, authorize('admin', 'librarian'), (req, res) => {
  ...
  // DEF-014: reject if membership has expired by date even if status is still 'active'
  if (member.membership_expiry && new Date(member.membership_expiry) < new Date()) {
    return res.status(400).json({ error: 'Member membership has expired. Please renew before borrowing.' });
  }
...
app.post('/api/loans/:id/return', authenticate, authorize('admin', 'librarian'), (req, res) => {
...
app.get('/api/loans', authenticate, (req, res) => {
  ...
  res.json({ loans: db.prepare(sql).all(...params) });
});

app.get('/api/loans/export', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const rows = db.prepare(
...
app.get('/api/loans/:id', authenticate, (req, res) => {
  ...
  `).get(req.params.id);
  if (!loan) return res.status(404).json({ error: 'Loan not found' });
  if (req.user.role === 'public' && loan.user_id !== req.user.id) {
...
app.post('/api/reservations', authenticate, (req, res) => {
  ...
  const book = db.prepare('SELECT * FROM books WHERE id = ?').get(book_id);
  if (!book) return res.status(404).json({ error: 'Book not found' });
  const existing = db.prepare("SELECT id FROM reservations WHERE user_id = ? AND book_id = ? AND status = 'pending'").get(req.user.id, book_id);
  if (existing) return res.status(409).json({ error: 'You already have a pending reservation for this book' });
  const activeLoad = db.prepare("SELECT id FROM loans WHERE user_id = ? AND book_id = ? AND status = 'active'").get(req.user.id, book_id);
...
app.delete('/api/reservations/:id', authenticate, (req, res) => {
  ...
  if (req.user.role === 'public' && reservation.user_id !== req.user.id) {
    return res.status(403).json({ error: 'Access denied' });
  }
...
app.post('/api/reservations/:id/fulfill', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const reservation = db.prepare('SELECT * FROM reservations WHERE id = ?').get(req.params.id);
  if (!reservation) return res.status(404).json({ error: 'Reservation not found' });
...
app.get('/api/events', (req, res) => {
  ...
  res.json({ events: db.prepare(sql).all() });
});

app.get('/api/events/:id', (req, res) => {
  const event = db.prepare('SELECT * FROM events WHERE id = ?').get(req.params.id);
  if (!event) return res.status(404).json({ error: 'Event not found' });
  const registered = db.prepare('SELECT COUNT(*) AS c FROM event_registrations WHERE event_id = ?').get(req.params.id).c;
  res.json({ event, registered });
});

app.post('/api/events', authenticate, authorize('admin', 'librarian', 'event_coordinator'), (req, res) => {
  ...
  logAudit(req.user.id, 'create', 'event', result.lastInsertRowid, title, req.ip);
  res.json({ success: true, id: result.lastInsertRowid });
});

app.put('/api/events/:id', authenticate, authorize('admin', 'librarian', 'event_coordinator'), (req, res) => {
  const event = db.prepare('SELECT * FROM events WHERE id = ?').get(req.params.id);
  if (!event) return res.status(404).json({ error: 'Event not found' });
  ...
  logAudit(req.user.id, 'update', 'event', req.params.id, null, req.ip);
  res.json({ success: true });
});

app.delete('/api/events/:id', authenticate, authorize('admin', 'librarian', 'event_coordinator'), (req, res) => {
  const event = db.prepare('SELECT * FROM events WHERE id = ?').get(req.params.id);
  if (!event) return res.status(404).json({ error: 'Event not found' });
  db.prepare('DELETE FROM event_registrations WHERE event_id = ?').run(req.params.id);
  db.prepare('DELETE FROM events WHERE id = ?').run(req.params.id);
  logAudit(req.user.id, 'delete', 'event', req.params.id, event.title, req.ip);
  res.json({ success: true });
});

app.post('/api/events/:id/register', optionalAuth, (req, res) => {
  ...
  if (userId) {
    const duplicate = db.prepare('SELECT id FROM event_registrations WHERE event_id = ? AND user_id = ?').get(req.params.id, userId);
    if (duplicate) return res.status(409).json({ error: 'You are already registered for this event' });
  } else if (email) {
    const duplicate = db.prepare('SELECT id FROM event_registrations WHERE event_id = ? AND email = ?').get(req.params.id, email);
    if (duplicate) return res.status(409).json({ error: 'This email is already registered for this event' });
  }
  const regName = name || req.user?.name || null;
  const regEmail = email || req.user?.email || null;
  db.prepare('INSERT INTO event_registrations (event_id, user_id, name, email, phone) VALUES (?, ?, ?, ?, ?)')
    .run(req.params.id, userId, regName, regEmail, phone || null);
  res.json({ success: true });
});
...
app.post('/api/announcements', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const { title, body, category, featured, emergency, publish_at, expires_at } = req.body;
  // DEF-001: server-side field validation
...
app.put('/api/announcements/:id', authenticate, authorize('admin', 'librarian'), (req, res) => {
...
app.delete('/api/announcements/:id', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const ann = db.prepare('SELECT id FROM announcements WHERE id = ?').get(req.params.id);
  if (!ann) return res.status(404).json({ error: 'Announcement not found' });
...
app.get('/api/contact', authenticate, authorize('admin', 'librarian'), (req, res) => {
...
app.put('/api/contact/:id', authenticate, authorize('admin', 'librarian'), (req, res) => {
  ...
  if (!status || !['new','read','resolved'].includes(status)) {
    return res.status(400).json({ error: 'status must be one of: new, read, resolved' });
  }
  db.prepare('UPDATE contact_messages SET status = ? WHERE id = ?').run(status, req.params.id);
  res.json({ success: true });
});

app.post('/api/ask-librarian', contactLimiter, optionalAuth, (req, res) => {
  ...
  db.prepare(`INSERT INTO librarian_requests (user_id, name, email, request_type, topic, details) VALUES (?, ?, ?, ?, ?, ?)`)
    .run(req.user?.id || null, name, email, request_type || null, topic || null, details);
  res.json({ success: true });
});

app.get('/api/ask-librarian', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const { status } = req.query;
...
app.put('/api/ask-librarian/:id', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const req_ = db.prepare('SELECT id FROM librarian_requests WHERE id = ?').get(req.params.id);
  ...
  db.prepare('UPDATE librarian_requests SET status = ? WHERE id = ?').run(status, req.params.id);
  res.json({ success: true });
});

// ---------- USERS / MEMBERSHIP (admin) ----------
...
app.get('/api/users/export', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const rows = db.prepare(
...
app.get('/api/users/:id', authenticate, (req, res) => {
  if (req.user.role === 'public' && req.user.id !== parseInt(req.params.id)) {
    return res.status(403).json({ error: 'Access denied' });
  }
  const user = db.prepare(
...
app.put('/api/users/:id', authenticate, (req, res) => {
  ...
  if (!user) return res.status(404).json({ error: 'User not found' });
  // Members can only update their own contact info; admins can update more
  ...
  logAudit(req.user.id, 'update_user', 'user', targetId, null, req.ip);
  res.json({ success: true });
});

app.post('/api/users/:id/approve', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const user = db.prepare('SELECT id FROM users WHERE id = ?').get(req.params.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  db.prepare("UPDATE users SET membership_status = 'active' WHERE id = ?").run(req.params.id);
  logAudit(req.user.id, 'approve_membership', 'user', req.params.id, null, req.ip);
  res.json({ success: true });
});

app.post('/api/users/:id/suspend', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const user = db.prepare('SELECT id FROM users WHERE id = ?').get(req.params.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  db.prepare("UPDATE users SET membership_status = 'suspended' WHERE id = ?").run(req.params.id);
  logAudit(req.user.id, 'suspend_membership', 'user', req.params.id, null, req.ip);
  res.json({ success: true });
});

app.post('/api/users/:id/role', authenticate, authorize('admin'), (req, res) => {
  const { role } = req.body;
  if (!['public', 'librarian', 'admin', 'event_coordinator'].includes(role)) {
    return res.status(400).json({ error: 'Invalid role' });
  }
  const user = db.prepare('SELECT id FROM users WHERE id = ?').get(req.params.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  db.prepare('UPDATE users SET role = ? WHERE id = ?').run(role, req.params.id);
  logAudit(req.user.id, 'change_role', 'user', req.params.id, role, req.ip);
  res.json({ success: true });
});

// ---------- DASHBOARD STATS ----------
app.get('/api/stats', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const stats = {
    total_books: db.prepare('SELECT COUNT(*) AS c FROM books').get().c,
    total_copies: db.prepare('SELECT COALESCE(SUM(total_copies),0) AS c FROM books').get().c,
    available_copies: db.prepare('SELECT COALESCE(SUM(available_copies),0) AS c FROM books').get().c,
    total_members: db.prepare("SELECT COUNT(*) AS c FROM users WHERE role = 'public'").get().c,
    active_members: db.prepare("SELECT COUNT(*) AS c FROM users WHERE role = 'public' AND membership_status = 'active'").get().c,
    pending_members: db.prepare("SELECT COUNT(*) AS c FROM users WHERE role = 'public' AND membership_status = 'pending'").get().c,
    suspended_members: db.prepare("SELECT COUNT(*) AS c FROM users WHERE role = 'public' AND membership_status = 'suspended'").get().c,
    active_loans: db.prepare("SELECT COUNT(*) AS c FROM loans WHERE status = 'active'").get().c,
    overdue_loans: db.prepare("SELECT COUNT(*) AS c FROM loans WHERE status = 'active' AND due_date < datetime('now')").get().c,
    total_fines: db.prepare("SELECT COALESCE(SUM(fine_amount),0) AS c FROM loans WHERE fine_amount > 0").get().c,
    pending_reservations: db.prepare("SELECT COUNT(*) AS c FROM reservations WHERE status = 'pending'").get().c,
    upcoming_events: db.prepare("SELECT COUNT(*) AS c FROM events WHERE event_date >= datetime('now')").get().c,
    new_messages: db.prepare("SELECT COUNT(*) AS c FROM contact_messages WHERE status = 'new'").get().c,
    open_librarian_requests: db.prepare("SELECT COUNT(*) AS c FROM librarian_requests WHERE status = 'open'").get().c,
    popular_books: db.prepare(`
...
app.get('/api/koha/search', async (req, res) => {
  ...
  try {
    ...
    // Extract biblionumbers from entry IDs
    ...
    // Fetch MARCXML for each entry in parallel; each record is cached individually.
      ...
      items.slice(0, 20).map(async ({ biblionumber, title }) => {
        try {
          ...
          return {
            ...
            title: String(title || '').replace(/\s+,\s*$/, '').trim() || `Record ${biblionumber}`,
            author: '', isbn: '', publisher: '', publication_year: null,
            call_number: '', category: 'General', language: 'English',
            collection_type: 'koha', branch: 'Eastern University',
            opac_url: `${KOHA_BASE}/cgi-bin/koha/opac-detail.pl?biblionumber=${biblionumber}`,
            cover_image: '/images/book-default.svg', available_copies: 1, description: '',
          };
        } catch { return null; }
      })
    ...
    // DEF-013: log internally but never send internal network details to the client
...
const CHAT_RATE_LIMIT = rateLimit({
  windowMs: 5 * 60 * 1000,
  max: 30,
  standardHeaders: true,
  legacyHeaders: false,
...
function generateChatReply(message) {
  ...
  if (lower.includes('how many book') || lower.includes('total book') || lower.includes('collection size')) {
    try {
      ...
      return `Our local digital catalog currently holds ${count.toLocaleString()} titles. The integrated Koha OPAC with Eastern University and partner libraries contains many more.\n\nSearch the catalog at /catalog or visit the Koha OPAC directly.`;
    } catch (e) {}
  }
...
app.post('/api/chat', CHAT_RATE_LIMIT, optionalAuth, (req, res) => {
  ...
  try {
    ...
    const existing = db.prepare('SELECT id FROM chat_sessions WHERE session_id = ?').get(sid);
    if (!existing) {
      db.prepare('INSERT INTO chat_sessions (session_id, user_id, visitor_ip) VALUES (?, ?, ?)').run(sid, userId, ip);
    }

    // Store user message
    db.prepare('INSERT INTO chat_messages (session_id, role, content) VALUES (?, ?, ?)').run(sid, 'user', msg);
...
app.get('/api/chat/sessions', authenticate, authorize('admin', 'librarian'), (req, res) => {
  ...
  const total = db.prepare('SELECT COUNT(*) AS c FROM chat_sessions').get().c;
  res.json({ sessions, total });
});

app.get('/api/chat/sessions/:session_id', authenticate, authorize('admin', 'librarian'), (req, res) => {
  const messages = db.prepare(
    'SELECT * FROM chat_messages WHERE session_id = ? ORDER BY created_at ASC'
  ).all(req.params.session_id);
  const session = db.prepare('SELECT cs.*, u.full_name AS member_name FROM chat_sessions cs LEFT JOIN users u ON cs.user_id = u.id WHERE cs.session_id = ?').get(req.params.session_id);
  if (!session) return res.status(404).json({ error: 'Session not found' });
  res.json({ session, messages });
});

app.delete('/api/chat/sessions/:session_id', authenticate, authorize('admin'), (req, res) => {
  db.prepare('DELETE FROM chat_messages WHERE session_id = ?').run(req.params.session_id);
  db.prepare('DELETE FROM chat_sessions WHERE session_id = ?').run(req.params.session_id);
  res.json({ success: true });
});

// DEF-018: expose only the env flag so the frontend can hide dev-only UI
...
app.get('/admin', (req, res) => {
  ...
  try {
    ...
    if (isTokenRevoked(token)) return res.redirect('/login?next=/admin');
  } catch {
    return res.redirect('/login?next=/admin');
  }
