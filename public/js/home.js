...
function buildKohaUrl(q,idx){const base="https://batticaloa.dlp.gov.lk/cgi-bin/koha/opac-main.pl";const params=new URLSearchParams({q,idx});return`${base}?${params}`}

document.getElementById("opac-search").addEventListener("submit",e=>{e.preventDefault();const n=document.getElementById("search-q").value.trim(),t=document.getElementById("search-idx").value;n?window.open(buildKohaUrl(n,t),"_blank","noopener"):document.getElementById("search-q").focus()}),loadHome();
